...
## Universal Anti-Patterns (apply regardless of surface)

These aren't ergonomics judgment calls — they're correctness and clarity
bugs that happen to look like style. Apply them the same way on public and
internal code.

**Mutable default arguments** — a default list/dict is created once at
function definition, not per call, so it silently persists and accumulates
state across calls.
...
**Type checking with `type()`** — `type(obj) == list` fails for subclasses
and is stricter than almost anyone actually wants; `isinstance` is also the
form that supports checking against multiple types at once.
...
**Comparing to `None` with `==`** — `==` can be overridden by `__eq__` on a
class in ways that produce surprising results against `None`; `is` checks
identity, which is what a `None`-check actually means.
...
**Wildcard imports** — `from module import *` pollutes the namespace,
breaks static analysis and "go to definition," and makes it impossible to
tell where a name came from just by reading the file.
...
`SystemExit`, silently swallows bugs that should have surfaced, and gives
future-you nothing to debug from.
...
If one of these shows up on a **public** surface, treat it as higher
priority to fix than the same issue on an internal one — a bare `except`
swallowing an error inside a private helper is a bug for you to find later;
the same bare `except` behind a public function is a bug your users hit
with no way to see what actually happened.
